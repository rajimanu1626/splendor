import { GEM_COLORS, createPlayer, } from '../lib/game-engine/types';
import { getInitialTokens, getNobleCount, getTokenCount, canTakeThreeTokens, canTakeTwoTokens, canPurchaseCard, canReserveCard, calculateGemPayment, getClaimableNobles, shuffleDeck, findCardOnBoard, findCardInReserved, determineWinner, } from '../lib/game-engine/rules';
...
export class GameRoomLogic {
    ...
    constructor(room) {
        ...
        const players = setupPlayers.map((sp, i) => createPlayer(room.players[i].playerId, sp.name, sp.isAI, sp.aiDifficulty));
    ...
    applyTakeThreeTokens(colors) {
        const state = this.state;
        if (state.phase !== 'playing' || !canTakeThreeTokens(colors, state.board.tokens))
            return;
        const player = state.players[state.currentPlayerIndex];
    ...
    applyTakeTwoTokens(color) {
        const state = this.state;
        if (state.phase !== 'playing' || !canTakeTwoTokens(color, state.board.tokens))
            return;
        const player = state.players[state.currentPlayerIndex];
    ...
    applyPurchaseCard(cardId) {
        ...
        if (!card || !canPurchaseCard(player, card))
            return;
    ...
    handleAction(playerId, payload) {
        ...
        switch (payload.type) {
            case 'takeThreeTokens':
                ...
                if (!canTakeThreeTokens(payload.payload.colors, state.board.tokens))
            ...
            case 'takeTwoTokens':
                if (state.phase !== 'playing')
                    return 'Invalid phase';
                if (!canTakeTwoTokens(payload.payload.color, state.board.tokens))
                    return 'Cannot take 2 of that color';
                this.applyTakeTwoTokens(payload.payload.color);
                break;
            case 'purchaseCard':
                if (state.phase !== 'playing')

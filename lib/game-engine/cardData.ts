import type { DevelopmentCard, NobleCard, GemColor } from './types';
...
function c(
  ...
  const cost: Partial<Record<GemColor, number>> = {};
...
function n(id: string, diamond: number, sapphire: number, emerald: number, ruby: number, onyx: number): NobleCard {
  const requirements: Partial<Record<GemColor, number>> = {};
...
export const ALL_NOBLES: NobleCard[] = [
  n('n-01', 3, 3, 0, 0, 3),
  n('n-02', 0, 3, 3, 3, 0),
  n('n-03', 3, 0, 0, 3, 3),
  n('n-04', 0, 0, 4, 4, 0),
